...
func (c *defaultCache) Range(findNodeEntry func(string, string) bool) {
	...
	for nodeUUID, nodeName := range c.uuidsToNames {
		klog.V(2).Infof("Calling findNodeEntry func for node entry with nodeUUID %s and nodeName %s",
			nodeUUID, nodeName)

		if !findNodeEntry(nodeUUID, nodeName) {
			klog.V(2).Infof("findNodeEntry func returned false for node entry with nodeUUID %s and nodeName %s, "+
